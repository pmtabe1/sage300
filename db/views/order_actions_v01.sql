SELECT
  ...
  order_metrics.FULFILLQTY, order_metrics.OPENORDQTY, GraniteDR.dbo.[Document].ShippingReference AS REFERENCE,
  open_orders.PONUMBER, open_orders.REFERENCE AS SOREFERENCE, open_orders.MRCAMOUNT,
      (
        SELECT id
        FROM dbo.shipping_references
        WHERE (number = GraniteDR.dbo.[Document].ShippingReference COLLATE Latin1_General_BIN)
      ) AS REFID,
      (
        SELECT created_at
...
INNER JOIN GraniteDR.dbo.[Document] ON open_orders.ORDNUMBER = GraniteDR.dbo.[Document].Number COLLATE Latin1_General_BIN
...
GROUP BY open_orders.ITEM, open_orders.[DESC], open_orders.ORDUNIQ, open_orders.ORDNUMBER,
  open_orders.LOCATION, open_orders.CUSTOMER, open_orders.CUSTNAME, 
  ...
  order_metrics.FULFILLQTY, order_metrics.OPENORDQTY, GraniteDR.dbo.[Document].ShippingReference,
